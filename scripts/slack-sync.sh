...
#
# Environment:
#   SLACK_BOT_TOKEN   (SECRET, required to do anything) bot token, injected by the
#                     environment config exactly like GH_TOKEN / AWS keys. If unset
...
#                     API-Usage-Monitor, Congress-Trading-Shared, Autorotate). When set:
