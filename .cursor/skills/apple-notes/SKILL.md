...
name: apple-notes
description: >-
  Write owner-facing Apple Notes in the `Coding` folder (local on this Mac) — plans, designs, reviews, handoffs, rollouts, living Completion notes. Use whenever Cursor produces something the owner needs to read, not only when they say "Notes." Title [APP, Cursor] … with a refreshed timestamp.
---
...
```
[APP, Cursor] short topic
```

- Acronyms first, then `Cursor` (Title Case, not all-caps Slack tags).
- Multi-app: `[ST, CT, Cursor] …` (impact order).
