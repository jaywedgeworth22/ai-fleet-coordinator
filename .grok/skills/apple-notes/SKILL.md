...
name: apple-notes
description: >-
  Write owner-facing Apple Notes in the `Coding` folder (local on this Mac) — plans, designs, reviews, handoffs, rollouts, living Completion notes. Use whenever Grok produces something the owner needs to read, not only when they say "Notes." Title [APP, Grok] … with a refreshed timestamp.
---
...
```
[APP, Grok] short topic
```

- Acronyms first, then `Grok` (Title Case, not all-caps Slack tags).
- Multi-app: `[ST, CT, Grok] …` (impact order).
