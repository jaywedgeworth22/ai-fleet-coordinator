...
App hosted on Vercel, Coolify, or elsewhere, with no Workers / R2 / WAF / other Cloudflare products: put the zone on the **Usage.Jays.Services** Cloudflare account (login `mail@jays.services`).  That account already holds Usage-Monitor plus DNS-only zones `jaywedgeworth.com` and `jays.services`.
