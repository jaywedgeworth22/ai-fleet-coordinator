# Fleet DNS and Registrars

Owner playbook 2026-08-24 (Jay).  Cloudflare is the DNS manager for every fleet domain.  Registrar and Cloudflare account are separate.
...
|------|------------|---------|
| **Account** | A Cloudflare login / billing container.  It holds many zones. | **Usage.Jays.Services** (login `mail@jays.services`) |
| **Zone** | One apex domain's DNS inside that account.  A new app gets its own new zone. | `contactlogo.com` created on account Usage.Jays.Services |
| **Hostname** | A name inside a zone (or the zone apex) that answers DNS. | `usage.jays.services` (already a live hostname) |

**Usage.Jays.Services is a Cloudflare account name.**  It is not a zone and not a hostname.  `usage.jays.services` is an already-live hostname.  `jays.services` is an existing DNS-only zone on that same account.
...
App hosted on Vercel, Coolify, or elsewhere, with no Workers / R2 / WAF / other Cloudflare products: create a **new zone for that app's apex** on the **Usage.Jays.Services** Cloudflare account (login `mail@jays.services`).  That account already holds Usage-Monitor plus DNS-only zones `jaywedgeworth.com` and `jays.services`.
...
- Skill: [`dns-and-registrars`](fleet-skills/dns-and-registrars/SKILL.md)
- New app: [ONBOARDING-NEW-APP.md](ONBOARDING-NEW-APP.md)
