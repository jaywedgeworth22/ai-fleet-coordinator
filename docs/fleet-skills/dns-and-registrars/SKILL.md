...
description: >-
  Fleet DNS and registrar playbook.  Cloudflare is DNS for every fleet domain;
  registrar and Cloudflare account are separate.  Usage.Jays.Services is a
  Cloudflare account name, not the hostname usage.jays.services.  Create a new
  zone for each app apex on that account.  Use when adding a domain, linking a
  host, choosing a Cloudflare account, or touching nameservers.  Do not buy
  domains, mint accounts, or enable paid Cloudflare products without Jay.
---
