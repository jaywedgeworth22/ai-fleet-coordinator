...
| ai-fleet-coordinator / machine infra | `ai-fleet-coordinator` or `fleet-infra` | AFC | `~/apps/fleet-monet` (or a `~/apps/fleet-monet-<lane>` worktree) | `~/apps/FLEET-INFRA-EFFORT-LOG.md` |
...
1. **THE BOARD** — `board list --app <app>` then `board claim <id> --by MONET --env Mac --where "~/apps/<lane> @ monet/<slug>"`.  If nothing exists: `board file --title "..." --app <app> --severity P1 --by MONET --env Mac --where "..." --desc "..."`.
...
```bash
AGENT_TAG=MONET /Users/jay/apps/agent-sync-websocket.py --post "[MONET] sync-1
repo: <project>
claim: monet/<slug>
state: WIP
