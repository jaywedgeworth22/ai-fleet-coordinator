---
name: unstick-pr
description: Diagnose and repair a PR that will not merge — phantom vs real conflicts, unresolved review threads, CI dispatch misses, required-check failures, and known flakes. Use when mergeable is false, auto-merge sits idle, GitHub says CONFLICTING/BLOCKED/DIRTY, or a Fx/peer PR is green but stuck.
---

# Unstick a blocked PR (FX)
...
**Phantom fix:** merge `origin/main` in the Fx worktree and push a fresh head SHA.

```bash
cd ~/apps/<prefix>-fx   # never ~/Code/<repo>
git fetch origin
git merge origin/main --no-edit
git push origin fx/<slug>
```
