...
name: apple-notes
description: >-
  Write owner-facing Apple Notes in the `Coding` folder (local on this Mac) — plans, designs, reviews, handoffs, rollouts, living Completion notes. Use whenever Fx produces something the owner needs to read, not only when they say "Notes." Title [APP, Fx] … with a refreshed timestamp.
---
...
- Acronyms first, then `Fx` (Title Case, not all-caps Slack tags).
- Multi-app: `[ST, CT, Fx] …` (impact order).
