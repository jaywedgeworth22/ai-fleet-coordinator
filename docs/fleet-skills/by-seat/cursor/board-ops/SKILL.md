...
board list --app congress-trade --mine CURSOR
board show <id>
```
...
board file --title "Scout drops Senate rows on 502" --app congress-trade \
  --severity P1 --by CURSOR --env Mac --where "~/apps/congress-cursor @ cursor/fix" \
  --desc "path:line + repro"

board claim <id> --by CURSOR --env Mac --where "~/apps/congress-cursor @ cursor/fix"

board comment <id> --by CURSOR --text "Verified on main; the shared helper is right."
...
2. **While working:** keep `--by CURSOR`, `--env Mac`, and `--where "worktree @ branch"` accurate.
