...
board list --app congress-trade --mine CODEX
board show <id>
```
...
board file --title "Scout drops Senate rows on 502" --app congress-trade \
  --severity P1 --by CODEX --env Mac --where "~/apps/congress-codex @ codex/fix" \
  --desc "path:line + repro"

board claim <id> --by CODEX --env Mac --where "~/apps/congress-codex @ codex/fix"
...
2. **While working:** keep `--by CODEX`, `--env Mac`, and `--where "worktree @ branch"` accurate.
