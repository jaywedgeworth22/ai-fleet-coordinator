...
name: apple-notes
description: >-
  Write owner-facing Apple Notes in the `Coding` folder (local on this Mac) — plans, designs, reviews, handoffs, rollouts, living Completion notes. Use whenever Codex produces something the owner needs to read, not only when they say "Notes." Title [APP, Codex] … with a refreshed timestamp.
---
...
```
[APP, Codex] short topic
```

- Acronyms first, then `Codex` (Title Case, not all-caps Slack tags).
- Multi-app: `[ST, CT, Codex] …` (impact order).
