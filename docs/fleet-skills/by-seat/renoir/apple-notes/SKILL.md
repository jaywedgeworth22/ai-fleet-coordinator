...
name: apple-notes
description: >-
  Write owner-facing Apple Notes in the `Coding` folder (local on this Mac) — plans, designs, reviews, handoffs, rollouts, living Completion notes. Use whenever Renoir produces something the owner needs to read, not only when they say "Notes." Title [APP, Renoir] … with a refreshed timestamp.
---
...
```
[APP, Renoir] short topic
```

- Acronyms first, then `Renoir` (Title Case, not all-caps Slack tags).
- Multi-app: `[ST, CT, Renoir] …` (impact order).
