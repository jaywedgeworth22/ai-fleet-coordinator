...
board list --app congress-trade --mine RENOIR
board show <id>
```
...
board file --title "Scout drops Senate rows on 502" --app congress-trade \
  --severity P1 --by RENOIR --env Mac --where "~/apps/congress-renoir @ renoir/fix" \
  --desc "path:line + repro"

board claim <id> --by RENOIR --env Mac --where "~/apps/congress-renoir @ renoir/fix"

board comment <id> --by RENOIR --text "Verified on main; the shared helper is right."
...
2. **While working:** keep `--by RENOIR`, `--env Mac`, and `--where "worktree @ branch"` accurate.
