...
If the box is gating several lanes, post `[MONET] gating now` with `repo:` (not `->FLEET` unless every Grok Bot seat must spend time).  Coordinator/ops self-id is `AFC`.
