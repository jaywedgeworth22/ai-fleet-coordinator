...
board list --app congress-trade --mine AG
board show <id>
```
...
board file --title "Scout drops Senate rows on 502" --app congress-trade \
  --severity P1 --by AG --env Mac --where "~/apps/congress-antigravity @ ag/fix" \
  --desc "path:line + repro"

board claim <id> --by AG --env Mac --where "~/apps/congress-antigravity @ ag/fix"

board comment <id> --by AG --text "Verified on main; the shared helper is right."
...
2. **While working:** keep `--by AG`, `--env Mac`, and `--where "worktree @ branch"` accurate.
