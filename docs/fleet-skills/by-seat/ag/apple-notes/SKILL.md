...
name: apple-notes
description: >-
  Write owner-facing Apple Notes in the `Coding` folder (local on this Mac) — plans, designs, reviews, handoffs, rollouts, living Completion notes. Use whenever Antigravity produces something the owner needs to read, not only when they say "Notes." Title [APP, Antigravity] … with a refreshed timestamp.
---
...
```
[APP, Antigravity] short topic
```

- Acronyms first, then `Antigravity` (Title Case, not all-caps Slack tags).
- Multi-app: `[ST, CT, Antigravity] …` (impact order).
