...
Later rounds on a merged PR are mostly noise.  Triage genuine hazards; surface the rest to the owner (Notes + board comment).  Grok Build's job on these is the security/contract read, not infinite bot ping-pong.
