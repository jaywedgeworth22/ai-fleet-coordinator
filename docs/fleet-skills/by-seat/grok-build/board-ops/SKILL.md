...
board list --app congress-trade --mine GROK-BUILD
board show <id>
```
...
board file --title "Scout drops Senate rows on 502" --app congress-trade \
  --severity P1 --by GROK-BUILD --env Mac --where "~/apps/congress-grok-build @ grok-build/fix" \
  --desc "path:line + repro"

board claim <id> --by GROK-BUILD --env Mac --where "~/apps/congress-grok-build @ grok-build/fix"

board comment <id> --by GROK-BUILD --text "Verified on main; the shared helper is right."
...
2. **While working:** keep `--by GROK-BUILD`, `--env Mac`, and `--where "worktree @ branch"` accurate.
