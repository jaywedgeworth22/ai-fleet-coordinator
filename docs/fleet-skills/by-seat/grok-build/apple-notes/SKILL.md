...
name: apple-notes
description: >-
  Write owner-facing Apple Notes in the `Coding` folder (local on this Mac) — plans, designs, reviews, handoffs, rollouts, living Completion notes. Use whenever Grok Build produces something the owner needs to read, not only when they say "Notes." Title [APP, Grok Build] … with a refreshed timestamp.
---

# Apple Notes (GROK-BUILD)
...
```
[APP, Grok Build] short topic
```

- Acronyms first, then `Grok Build` (Title Case, not all-caps Slack tags).
- Multi-app: `[ST, CT, Grok Build] …` (impact order).
