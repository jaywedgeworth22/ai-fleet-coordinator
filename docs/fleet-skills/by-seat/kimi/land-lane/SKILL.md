---
name: land-lane
description: Land a Kimi feature branch to main across the fleet — seat worktree, docs, verification gate, PR, auto-merge, then closeout. Use when finishing a unit, opening a PR, merging, or when the owner says land/ship/commit/push. Never wait for the owner to ask. Covers apps with and without scripts/land.sh.
---
...
Seat: **KIMI**.  Branch: `kimi/<slug>`.  Never `claude/`.  Never `monet/`.  Never land from `~/Code/<repo>` or from branch `main`.

## Preconditions

1. You are in your Kimi worktree (`~/apps/<prefix>-kimi` or `~/apps/<prefix>-kimi-<lane>`).  See `session-start`.
...
4. Substantial owner-facing work: living Apple Note via the `apple-notes` skill, title `[APP, Kimi] …`.
...
If the box is gating several lanes, post `[KIMI] gating now` with `repo:` (not `->FLEET` unless every Grok Bot seat must spend time).  Coordinator/ops self-id is `AFL`.
