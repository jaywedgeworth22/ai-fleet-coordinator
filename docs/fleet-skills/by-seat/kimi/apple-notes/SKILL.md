...
name: apple-notes
description: >-
  Write owner-facing Apple Notes in the `Coding` folder (local on this Mac) — plans, designs, reviews, handoffs, rollouts, living Completion notes. Use whenever Kimi produces something the owner needs to read, not only when they say "Notes." Title [APP, Kimi] … with a refreshed timestamp.
---
...
```
[APP, Kimi] short topic
```

- Acronyms first, then `Kimi` (Title Case, not all-caps Slack tags).
- Multi-app: `[ST, CT, Kimi] …` (impact order).
