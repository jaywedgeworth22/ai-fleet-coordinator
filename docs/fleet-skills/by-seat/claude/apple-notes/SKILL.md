...
name: apple-notes
description: >-
  Write owner-facing Apple Notes in the `Coding` folder (local on this Mac) — plans, designs, reviews, handoffs, rollouts, living Completion notes. Use whenever Claude produces something the owner needs to read, not only when they say "Notes." Title [APP, Claude] … with a refreshed timestamp.
---
...
```
[APP, Claude] short topic
```

- Acronyms first, then `Claude` (Title Case, not all-caps Slack tags).
- Multi-app: `[ST, CT, Claude] …` (impact order).
