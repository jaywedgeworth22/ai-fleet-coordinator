...
board list --app congress-trade --mine GROK
board show <id>
```
...
board file --title "Scout drops Senate rows on 502" --app congress-trade \
  --severity P1 --by GROK --env Mac --where "~/apps/congress-grok @ grok/fix" \
  --desc "path:line + repro"

board claim <id> --by GROK --env Mac --where "~/apps/congress-grok @ grok/fix"

board comment <id> --by GROK --text "Verified on main; the shared helper is right."
...
2. **While working:** keep `--by GROK`, `--env Mac`, and `--where "worktree @ branch"` accurate.
