...
- **2026-09-01 — GROK — IN PROGRESS — DSH Dock WKWebView (branch `grok/dsh-dock-webview`, worktree `~/apps/fleet-grok-dsh-webview`).**  Chrome `--app` spawned a new window every click.  Replaced with a real WKWebView app so the Dock shows a running-dot and a second click focuses the same window (GitHub.app pattern).
...
- **2026-09-02 — CLAUDE — DEPLOYED — Fleet RAG as a memory platform every agent commits lessons to.**  Board `5c27dffd` (+ `0f9a13b1` doc diet via #175).  Worktree `~/apps/fleet-claude-rag` @ `claude/fleet-rag-platform`.  Search: in-process grouping (one hit per doc, adaptive window), lesson-boost prefetch, cross-encoder rerank via new Coolify `tei-reranker` (MiniLM, 0.5 s / 30 candidates; bge-reranker-v2-m3 rejected at 54 s / 20 pairs on CPU); golden 30→75, eval 0.71/0.84/0.77 → **0.76/0.92/0.83** (R@1/R@5/MRR).  Corpus: mirror dedupe at the source + `--prune` (38.7k → 36.9k points).  Write path: contribute near-duplicate guard, `recall digest`, `recall doctor --platforms --box`, Claude Code SessionStart/Stop hooks (once-per-session nudge).  Cloud: Hetzner-side `recall-api` (REST + MCP, bearer, gitleaks-gated) at https://recall.jays.services under the agents.jays.services Access app + /health bypass + proxied DNS (Access managed with the Global key; scoped tokens have no Access scope).  Oracle routines: nightly `--prune`, weekly doctor/eval/digest → owner note.  Rollout: `docs/rollouts/2026-09-02-fleet-rag-memory-platform.md`.
...
- **2026-09-01 — GROK — IN PROGRESS — Mine chat logs + extra markdown into fleet-agents RAG.**  Board `ef4df7cb`.  Claimed Tue, Sep 1, 2026.  Worktree `~/apps/fleet-grok-rag-mine` @ `grok/rag-mine-chats-docs`.  AFL #170.  Staging `~/apps/fleet-rag/mined/`: Claude 7224, Grok 36783, Cursor 1164, Codex 555, Gemini 4446, Kimi 18, BotFleet 148, extra markdown 2158, distilled lessons 4012 (75k JSONL lines, scrubbed, not git).  Generators: `chat-log` + expanded `doc` walker.  Unittest 63 OK.  Do not start a second ingest while pid 81666 holds `ingest.lock`.  Install runtime after merge + lock release.  Rollout `docs/rollouts/2026-09-01-rag-mine-chats-docs.md`.
