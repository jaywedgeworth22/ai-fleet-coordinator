...
- **2026-09-01 — GROK — IN PROGRESS — Mine chat logs + extra markdown into fleet-agents RAG.**  Board `ef4df7cb`.  Worktree `~/apps/fleet-grok-rag-mine` @ `grok/rag-mine-chats-docs`.  Source generators landed: `chat-log` (Claude/Grok/Cursor/Codex/Gemini user+assistant text) plus expanded `doc` walker (fleet app README/AGENTS/STATUS/CLAUDE + docs, `~/apps/*.md` except effort logs, `~/.grok/docs` and grok skills).  Unit tests pass (`python3 -m unittest fleet_rag.tests.test_sources`).  Do not start ingest while pid 81666 holds `ingest.lock`.  Do not copy into `~/apps/fleet-rag` runtime until `install-fleet-rag.sh`.  Gaps: BotFleet events/Kimi chats not parsed.
