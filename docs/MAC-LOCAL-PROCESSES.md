...
| `com.jay.botfleet-imessage-relay` | Always-on | Monitors iMessage group chats for BotFleet bots and bidirectionally relays to the harness.  Script: `~/apps/botfleet-imessage-relay.py`.  launchd uses Xcode `Python.app` (Full Disk Access) like `com.jay.imessage-grok`.  Reloaded 2026-09-01 GROK; mapped 12 bot chats.  Log writes only to stderr (launchd already captures `~/Library/Logs/botfleet/imessage-relay.log`) so lines are not doubled. | **Up** |
