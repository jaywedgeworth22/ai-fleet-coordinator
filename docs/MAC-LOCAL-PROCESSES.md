...
| **pm2 `agy-acp`** | Always-on | Antigravity ACP on `127.0.0.1:8765` (loopback bind 2026-08-21).  Pinned `~/apps/agy-acp-runtime` + `start.sh`.  Public `acp.jays.services` is WAF-blocked (Access is not enabled on the Usage.Jays.Services account).  Phone/Conductor uses Shellular, not this hostname. | **Up** |
...
| `com.jay.mac-cleanup` | every 4 h (14400s) | Regenerable caches only: Xcode DeviceSupport/DerivedData, unavailable simulators, package caches, aged agent transcripts.  Worktree retirement stays with disk-janitor (idle + clean + KEEP_RE).  Does not wipe `~/.grok/worktrees` or live CoreSimulator devices.  Does not SSH Coolify.  **Wipes `~/.npm/_npx`.** |
