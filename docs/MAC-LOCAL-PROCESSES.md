...
Last inventory: Tue, Sep 1, 2026 ~4:56am CT (Grok).  Added `com.jay.mac-resource-watch` (every 5 min, disk/RAM/CPU → cleanup + Housekeeper webhook).  Janitor warn 80G / pressure 65G.  Disk janitor now walks all fleet Code repos (was ST/CT/CTS only), protects unsuffixed standing lanes + runtimes, reaps clean kimi-named / nested / `/tmp` trees, and uses 2-day idle when free < 50G.  Live `~/.claude-disk-janitor/janitor.sh`; tracked `scripts/disk-janitor.sh`.  `mac-collab-writeback` is in the ecosystem (15 pm2 apps) and watch expected list.  Hardened after the first-run loop (updated_at from sync POSTs → gh close/reopen of ~1700 issues/cycle; lossy md re-render).  Writeback now bootstraps an applied-status map, edits live logs surgically, uses REST + current-state check for Issues, and does not git-commit `~/Code/<repo>`.  Tracked copies: `ai-fleet-coordinator/scripts/mac-process-watch.sh` + `scripts/pm2-ecosystem.config.cjs` + `scripts/mac-collab/write_back.py` + `scripts/grok-leader.sh` + `scripts/mac-status.sh` + `scripts/disk-janitor.sh`.  Command of record: `bash ~/apps/mac-status.sh`.
...
| `~/apps/dsh-runtime/` | Pinned DeepSeek Harness (`@deepseek-ai/dsh`).  **Never `npx @deepseek-ai/dsh`.**  Web UI is always-on pm2 `dsh-web` on **:3080** (~12 MB RSS / 0% CPU idle — cheap enough to leave up).  Dock launcher: `~/Applications/DeepSeek Harness Web.app` (Chrome `--app`, no Terminal).  Rebuild: `~/apps/dsh-runtime/install-dock-app.sh`.  `dsh-acp.sh` is Shellular id `deepseek`. |
...
| `~/apps/grok-acp-runtime/grok-drive.py` | On-demand.  Any-seat CLI: `list` / `peek` / `tail` / `prompt` / `await` / `cancel` / `new`.  `list` merges `active_sessions.json` `live=true` plus `turnState` and `pendingTool`.  Prompt uses `session/resume`.  Refuses `$GROK_SESSION_ID` unless `--self`.  Tracked `scripts/grok-acp-runtime/grok-drive.py`. |
