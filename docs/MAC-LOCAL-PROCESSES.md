...
| `com.jays.codex-usage-collector` | every 15 min | Codex CLI session JSONL tokens → Usage Monitor ingest (estimated API-equivalent).  Default lookback 180 days (not UTC month start).  On-demand: `npm run codex:collect -- --dry-run` or `--days 400` to backfill. |
| `com.jays.grok-usage-collector` | every 15 min | Grok Build `updates.jsonl` turn_completed tokens + costUsdTicks → Usage Monitor ingest (estimated API-equivalent).  Default lookback 180 days (not UTC month start).  On-demand: `npm run grok:collect -- --dry-run` or `--days 400` to backfill. |
