...
| **pm2 `grok-acp`** | Always-on | Grok ACP WebSocket on `127.0.0.1:12419` (`/ws`).  Pinned `~/apps/grok-acp-runtime`.  Native `~/.grok/bin/grok agent --always-approve --no-leader serve`.  `start.sh` sets `GROK_HOME` to `acp-home/` (stripped config, no plugins, Claude/Cursor MCPs off) so Conductor sessions do not load the TUI kitchen sink.  `acp-client.py --mcp-server NAME` expands names from user `~/.grok/config.toml`.  Empty list = no MCPs.  TUI / grok-leader keep `~/.grok`.  **Never bind `:2419`**.  `--leader serve` does not listen.  Token in `~/.secrets/grok-acp.env` (never print).  Added 2026-08-18.  Per-session MCP pick 2026-08-29. | **Up** |
...
| `~/apps/grok-acp-runtime/acp-client.py` | Conductor WS client for **new** sessions on `:12419`.  `--mcp-server NAME` from user `~/.grok/config.toml`. |
