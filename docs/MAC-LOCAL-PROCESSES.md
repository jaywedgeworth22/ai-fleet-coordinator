...
| `~/apps/dsh-runtime/` | Pinned DeepSeek Harness (`@deepseek-ai/dsh`).  **Never `npx @deepseek-ai/dsh`** — that poisoned pm2.  Web UI is always-on pm2 `dsh-web` on **:3080** (Tailscale `https://macbook.boa-roygbiv.ts.net:3080`).  `dsh-acp.sh` is Shellular id `deepseek` (ACP stdio; `dsh acp` is not a command).  Auth: `~/.dsh/.credentials.yaml`, not `agents.json`.  Bridge 1.3.0 detaches ACP stdin, heartbeats `[working… Ns]`, and kills the process group so iOS does not stay on Thinking.  Tracked: `scripts/dsh-acp.py` + `scripts/dsh-acp.sh` + `scripts/dsh-runtime/start-web.sh`. |
...
| `~/apps/grok-acp-runtime/start.sh` | pm2 `grok-acp` entry: `--no-leader serve --bind 127.0.0.1:12419`.  Sources `~/.secrets/grok-acp.env`.  Sets `GROK_HOME` to `acp-home/` (auth.json + trusted_folders.toml symlinks only). |
| `~/apps/grok-acp-runtime/acp-client.py` | Conductor WS client for **new** sessions on `:12419`.  `--mcp-server NAME` from user `~/.grok/config.toml`.  Auto-approves `session/request_permission` (offered allow option) and implements ACP `terminal/*`. |
| `~/apps/grok-acp-runtime/mcp_catalog.py` | Name → ACP `mcpServers` object.  Never prints env/header values. |
| `~/apps/grok-acp-runtime/acp-home-config.toml` | Stripped grok-acp config (no `[mcp_servers]`, plugins empty). |
