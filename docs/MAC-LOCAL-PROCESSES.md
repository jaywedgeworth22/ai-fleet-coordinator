...
Last inventory: Mon, Sep 1, 2026 ~11:30pm CT (Grok).  Added hourly `com.jay.grok-idle-unload` (session/close live Grok chats idle >36h; transcripts kept).  Hetzner swap target 16GiB + swappiness 20 via `scripts/host/ensure-swap.sh`.  Added on-demand `~/apps/fleet-rag/recall` + `scripts/install-fleet-rag.sh` (MCP registration, PATH like `board`).  Box Qdrant snapshot/health helpers tracked under `scripts/hetzner/`.  Added `com.jay.mac-resource-watch` (every 5 min, disk/RAM/CPU → cleanup + Housekeeper webhook).  Janitor warn 80G / pressure 65G.  Disk janitor now walks all fleet Code repos (was ST/CT/CTS only), protects unsuffixed standing lanes + runtimes, reaps clean kimi-named / nested / `/tmp` trees, and uses 2-day idle when free < 50G.  Live `~/.claude-disk-janitor/janitor.sh`; tracked `scripts/disk-janitor.sh`.  `mac-collab-writeback` is in the ecosystem (15 pm2 apps) and watch expected list.  Hardened after the first-run loop (updated_at from sync POSTs → gh close/reopen of ~1700 issues/cycle; lossy md re-render).  Writeback now bootstraps an applied-status map, edits live logs surgically, uses REST + current-state check for Issues, and does not git-commit `~/Code/<repo>`.  Tracked copies: `ai-fleet-coordinator/scripts/mac-process-watch.sh` + `scripts/pm2-ecosystem.config.cjs` + `scripts/mac-collab/write_back.py` + `scripts/grok-leader.sh` + `scripts/mac-status.sh` + `scripts/disk-janitor.sh`.  Command of record: `bash ~/apps/mac-status.sh`.
...
| `com.jay.botfleet-server` | Always-on | BotFleet harness server on loopback `127.0.0.1:8799` and webhook `8800`. Directory: `~/apps/botfleet-grok`. | **Up** |
| `com.jay.botfleet-imessage-relay` | Always-on | Monitors iMessage group chats for all 12 BotFleet bots and bidirectionally relays messages to/from BotFleet. Script: `~/apps/botfleet-imessage-relay.py`. | **Up** |
...
| `com.jay.grok-idle-unload` | hourly (3600s) | **Scheduled, not always-on.**  `session/close` on **live** Grok TUI chats idle >36h so their MCP tool processes exit.  Does **not** `/delete` transcripts.  Skips working / needs-input / pendingTool / `$GROK_SESSION_ID`.  Also SIGTERMs orphan MCP children whose session is no longer live and is missing or also idle >36h.  Live `~/apps/grok-acp-runtime/grok-idle-unload.py`.  Tracked `scripts/grok-acp-runtime/grok-idle-unload.py` + `scripts/launchd/com.jay.grok-idle-unload.plist`.  On-demand: `python3 ~/apps/grok-acp-runtime/grok-idle-unload.py --dry-run`. |
...
| `~/apps/grok-acp-runtime/grok-idle-unload.py` | On-demand / hourly launchd.  Close live chats idle >36h.  `--dry-run` lists only.  Tracked `scripts/grok-acp-runtime/grok-idle-unload.py`. |
...
| `~/apps/update-botfleet.sh` | On-demand. Pulls latest `botfleet-grok` main, packages and installs new BotFleet macOS release to `/Applications/BotFleet.app`, automatically signs with stable local Developer ID / Development certificate to preserve macOS Keychain secret storage access across rebuilds. |
...
| `~/apps/ios-fleet/ship-testflight.sh` | Archive + upload one iOS app to TestFlight.  Consults `build-window.sh` and re-execs itself under `nice -n 10` when the answer is `background`.  Never blocked, only deprioritised.  `IOS_SHIP_RENICED` guards against a re-exec loop; `IOS_SHIP_PRIORITY=normal` forces full speed.  **TODO (Sentry Size Analysis, 2026-09-01):** after a successful archive, upload the XCArchive via `sentry-cli` / Fastlane `sentry_upload_build` (100 builds/month included).  Do not invent a new LaunchAgent; hook this path only. |
