...
**Use the 3 days:** land PR B (A2), evaluate condense-first, then decide — Builder ($20/mo, hard-capped), extended free fuses, or accelerated self-host — with measured numbers, before the real trial end.
...
**Do today**
1. Push + land PR B from this Mac (A2).  2. Rotate MAC_COLLAB_TOKEN and the Aug 21 leak batch (B-security).  3. Pinecone: use the deliberate 3-day buffer — evaluate condense-first post-PR-B, decide Builder vs fuses vs self-host before the real trial end ~Aug 30 (A1).
