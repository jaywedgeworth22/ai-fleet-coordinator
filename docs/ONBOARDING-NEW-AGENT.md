...
| iOS copy | Title Case nav / ASC listing copy: `FLEET-UI-COPY.md`. TestFlight notes never include agent names. Compiler / `GB-COMPILE` owns iOS ship on GitHub-hosted `macos-latest` only — do not run `xcodebuild` / TestFlight / `ios-ship-now` / `--force-ship` from a fleet seat. |
