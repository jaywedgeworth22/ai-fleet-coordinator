...
| **Grok Bot (`GROK-BOT`)** | Grok Bot driving Cursor cloud agents. Distinct from Mac Grok TUI and from local Cursor. | `[GROK-BOT]` | `Grok Bot` | Cloud Cursor work is GROK-BOT, not `[CURSOR]` and not `[GROK]`. Prefix often `cursor/` in cloud. |
