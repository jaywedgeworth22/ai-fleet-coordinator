...
```bash
export AGENT_SEAT=<YOUR_SEAT>
export AGENT_TAG=<YOUR_TAG>
```
...
| App | Slack `repo:` | Acronym | seat worktree | Live board |
|-----|---------------|---------|----------------|------------|
| Socratic.Trade | `Socratic.Trade` | ST | `~/apps/trading-<seat>` | `~/apps/TRADING-EFFORT-LOG.md` |
| Congress.Trade | `Congress.Trade` | CT | `~/apps/congress-<seat>` | `~/apps/CONGRESS-TRADE-EFFORT-LOG.md` |
| Usage Monitor | `API-usage-monitor` | UM | `~/apps/usage-<seat>` | `~/apps/API-USAGE-MONITOR-EFFORT-LOG.md` |
...
| ai-fleet-coordinator / machine infra | `ai-fleet-coordinator` or `fleet-infra` | AFC | `~/apps/fleet-<seat>` (or a `~/apps/fleet-<seat>-<lane>` worktree) | `~/apps/FLEET-INFRA-EFFORT-LOG.md` |

As of 2026-08-20 only `~/apps/trading-<seat>` is guaranteed to exist.  Create a missing standing lane before editing:

```bash
git -C /Users/jay/Code/<Repo> worktree add -b <seat>/<slug> ~/apps/<prefix>-<seat>
```

Per-lane isolation is also fine: `~/apps/<prefix>-<seat>-<lane>`.  Inventory is `~/Code/ai-fleet-coordinator/fleet-apps.json`.  `scripts/setup-agent-lanes.sh` uses a different naming scheme (`Socratic.Trade-monet` / `agent/monet`) — do not run it for this seat.
...
1. **THE BOARD** — `board list --app <app>` then `board claim <id> --by <YOUR_TAG> --env Mac --where "~/apps/<lane> @ <seat>/<slug>"`.  If nothing exists: `board file --title "..." --app <app> --severity P1 --by <YOUR_TAG> --env Mac --where "..." --desc "..."`.
...
```bash
AGENT_TAG=<YOUR_TAG> /Users/jay/apps/agent-sync-websocket.py --post "[<YOUR_TAG>] sync-1
repo: <project>
claim: <seat>/<slug>
state: WIP
...
- Self-filter Slack on `[<YOUR_TAG>` when you run parallel agent lanes — sibling posts are for you too.
